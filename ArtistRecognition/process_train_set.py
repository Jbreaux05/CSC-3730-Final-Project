...
import numpy as np
from . import hdf5_getters as GETTERS
...
def process_filelist_train(filelist=None,testsongs=None,tmpfilename=None):
    ...
    if os.path.isfile(tmpfilename):
        print(f"ERROR: file '{tmpfilename}' already exists.")
        return
    ...
    output.create_earray(group,'feats',tables.Float64Atom(shape=()),(0,finaldim),'',
                        expectedrows=len(filelist))
    output.create_earray(group,'artist_id',tables.StringAtom(18,shape=()),(0,),'',
                        expectedrows=len(filelist))
    # iterate over files
    cnt_f = 0
    for f in filelist:
...
def train(nthreads,maindir,output,testsongs,trainsongs=None):
    ...
    h5 = tables.open_file(tmpfiles[0],'r')
    nrows = h5.root.data.artist_id.shape[0] * len(tmpfiles)
    h5.close()
    # create output
    output = tables.open_file(output, mode='a')
    group = output.create_group("/",'data','KNN MODEL FILE FOR ARTIST RECOGNITION')
    output.create_earray(group,'feats',tables.Float64Atom(shape=()),(0,90),'feats',
                        expectedrows=nrows)
    output.create_earray(group,'artist_id',tables.StringAtom(18,shape=()),(0,),'artist_id',
                        expectedrows=nrows)
...
if __name__ == '__main__':
    ...
    trainsongs = list(map(lambda x: fullpath_from_trackid(msd_dir,x[0]),data))
